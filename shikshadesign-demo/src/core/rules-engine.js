...
export class SectionValidator {
  ...
  static validateOutcomes(outcomes, stakeholders = []) {
    ...
    outcomes.forEach((outcome, idx) => {
      ...
      // LOGIC GUARDRAIL 1: Context Consistency
      // Check if outcome contradicts target group characteristics
      const text = outcome.statement?.toLowerCase() || '';
      if (text.includes('web development') || text.includes('coding') || text.includes('engineering')) {
          if (text.includes('adolescent') || text.includes('14-18') || text.includes('school')) {
             results.push(
                new ValidationResult(
                  `${prefix}.statement`,
                  SEVERITY.WARNING,
                  `Context Mismatch Risk: Advanced technical skills for school-age group`,
                  `Web development/Engineering is often too advanced for 14-18 year olds still in school. Consider age-appropriate digital literacy or coding basics instead.`
                )
             );
          }
      }

      // LOGIC GUARDRAIL 2: Outcome vs Indicator Mismatch
      // If outcome says "Employment" but indicator says "Pass Rate", that's a mismatch
      const indicators = outcome.indicators || [];
      indicators.forEach((ind, i) => {
          const indText = ind.name?.toLowerCase() || '';
          if (text.includes('jobs') || text.includes('employment') || text.includes('income')) {
              if (indText.includes('graduat') || indText.includes('pass') || indText.includes('enroll')) {
                   results.push(
                    new ValidationResult(
                      `${prefix}.indicators[${i}].name`,
                      SEVERITY.WARNING,
                      `Logic Gap: Indicator does not measure Outcome`,
                      `Your outcome is about 'Jobs/Income' but your indicator measures 'School Graduation'. These are not the same. Change indicator to 'Job Placement Rate' or 'Average Income'.`
                    )
                 );
              }
          }
      ...
      if (indicators.length === 0) {
        ...
        indicators.forEach((indicator, indIdx) => {
          ...
          } else if (indicator.metric.target == 100 && (text.includes('job') || text.includes('income'))) {
             // LOGIC GUARDRAIL 4: Realistic Targets
             results.push(
              new ValidationResult(
                `${prefix}.indicators[${indIdx}].metric.target`,
                SEVERITY.INFO,
                `Reality Check: 100% success rate is rare`,
                `Achieving 100% employment/income increase is very difficult. Donors often prefer conservative estimates (e.g., 70-80%).`
              )
            );
          }
  ...
  static validateActivities(activities, outputs = []) {
    ...
    activities.forEach((activity, idx) => {
      const prefix = `activities[${idx}]`;
       ...
       // LOGIC GUARDRAIL 3: Appropriateness Check
      const text = activity.description?.toLowerCase() || '';
      if (text.includes('engineering') || text.includes('full-stack') || text.includes('architecture')) {
          results.push(
            new ValidationResult(
                `${prefix}.description`,
                SEVERITY.WARNING,
                `Feasibility Check: Highly specialized training`,
                `Are you sure this target group has the prerequisites for '${activity.description}'? Ensure inputs (participants) match the difficulty level.`
            )
          );
      }
